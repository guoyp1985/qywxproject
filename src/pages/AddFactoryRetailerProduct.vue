  ...
  methods: {
    ...
    savedata (postdata) {
      ...
      if (!self.submitIng) {
        ...
        self.$http.post(`${ENV.BokaApi}/api/add/factoryretailerproduct`, postdata).then(res => {
          let data = res.data
          self.$vux.loading.hide()
          self.$vux.toast.show({
            text: data.error,
            ...
            onHide: () => {
              ...
              if (data.flag === 1) {
                ...
                self.$router.push({path: '/factoryRetailerProductList', query: rparams})
              }
            }
          })
        })
...
<style lang="less">
.add-factory-retailer-product-page{
  line-height: 1.4;
