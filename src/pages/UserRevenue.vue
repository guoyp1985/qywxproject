<template>
  <div class="containerarea font14 user-revenue-page" ref="revenueContainer">
    <div class="inhead">
    ...
    <div class="middle-content">
      ...
      <div class="align_center pb10 font30 bold" style="color:#FF6B63;">{{loginUser.waitcash}}</div>
        ...
        <div class="wechatShow" v-if="wechatShow">
          ...
          <div class="pt10 pb10 pr10 pl10">
            ...
            <div class="mt5 db-flex">
              <div class="flex_cell color-gray">可提现金额￥{{loginUser.waitcash}}</div>
  ...
  methods: {
    ...
    cashEvent (inputMoney, type) {
      if (!this.submitIng) {
        ...
        let waitcash = parseFloat(this.loginUser.waitcash.replace(/,/g, ''))
        if (money > waitcash) {
          this.$vux.toast.show({
            text: '提现金额不能超过可提现金额',
            width: '220px',
        ...
        this.$http.post(`${ENV.BokaApi}/api/accounting/cashMoney`, postData).then(res => {
          ...
          if (data.flag) {
            this.loginUser.waitcash = data.waitcash
    ...
    clickAll (type) {
      if (type === 'bank') {
        this.cashBankMoney = this.loginUser.waitcash
      } else {
        this.cashMoney = this.loginUser.waitcash
      }
    }
  },
  activated () {
    ...
    if (this.query.appid) {
      ...
      this.fromPage = encodeURIComponent('/userRevenue')
    }
  }
}
</script>
