...
export default {
  ...
  components: {
    Tab, TabItem, Swiper, SwiperItem, Search, Group, Popup, XImg, PopupHeader, Radio
  },
  data () {
    return {
      loginUser: {},
  ...
  methods: {
    ...
    priorityEvent (item, index) {
      ...
      this.$http.post(`${ENV.BokaApi}/api/retailer/sellerAction`,
          ...
          onHide: () => {
            if (data.flag === 1) {
              if (this.selectedIndex === 0) {
                this.tabdata1[index].priority = !this.tabdata1[index].priority
              } else if (this.selectedIndex === 2) {
                this.tabdata2[index].priority = !this.tabdata2[index].priority
              } else if (this.selectedIndex === 1) {
                this.tabdata3[index].priority = !this.tabdata3[index].priority
              }
            }
          }
        })
    ...
    toCard (item) {
      if (this.query.from) {
        jweixin.miniProgram.navigateTo({url: `${ENV.MiniRouter.addCard}?uid=${item.uid}`})
      }
    },
    btnDetail (index) {
    ...
    removeOrderActive () {
      this.dateClass = this.dateClass.replace(' active', '').replace('active', '')
      this.areaClass = this.areaClass.replace(' active', '').replace('active', '')
      this.sexClass = this.sexClass.replace(' active', '').replace('active', '')
    },
    dateOrder () {
      this.selectedArea = null
      this.selectedSex = null
      if (this.dateClass.indexOf('active') < 0) {
        this.removeOrderActive()
        this.dateClass = 'active'
        this.orderbyParams = { orderby: 'dateline' }
      } else {
        this.removeOrderActive()
        this.dateClass = ''
        this.orderbyParams = {}
    ...
    listSex () {
      ...
      this.tabdata1 = []
      this.selectedArea = null
      this.orderbyParams = {orderby: 'sex', sex: this.sexKey}
      this.selectedSex = this.sexParams[this.sexKey]
      this.getData1()
    },
    areaOrder () {
    ...
    listArea () {
      ...
      this.tabdata1 = []
      this.selectedSex = null
      this.orderbyParams = { orderby: 'province', province: this.areaArr[this.areaKey].value }
      this.selectedArea = this.orderbyParams.province
      this.getData1()
    },
    getData1 () {
      this.$vux.loading.show()
      let params = { tolevel: -1, pagestart: this.pagestart1, limit: this.limit }
      let keyword = this.searchword1
      if (typeof keyword !== 'undefined' && keyword && this.$util.trim(keyword) !== '') {
        this.searchresult1 = true
        params.keyword = keyword
      } else {
        this.searchresult1 = false
      }
      if (typeof this.orderby !== 'undefined' && this.orderby && this.$util.trim(this.orderby) !== '') {
        params.orderby = this.orderby
      }
      for (let key in this.orderbyParams) {
        params[key] = this.orderbyParams[key]
      }
      this.$http.get(`${ENV.BokaApi}/api/retailer/customerList`, {
        params: params
      }).then(res => {
        let data = res.data
        this.$vux.loading.hide()
        this.tabcount1 = data.count
        if (data.provinces.length === 0) {
          this.disAreaOrder = false
        } else {
          if (this.areaArr.length === 0) {
            for (let i = 0; i < data.provinces.length; i++) {
              this.areaArr.push({key: i, value: data.provinces[i]})
            }
          }
          this.disAreaOrder = true
        }
        ...
        this.distabdata1 = true
        if (this.tabdata1.length > 0) {
          this.disOrderArea = true
        } else if (!this.orderbyParams.orderby) {
          this.disOrderArea = false
        }
      })
    },
    getData2 () {
      this.$vux.loading.show()
      let keyword = this.searchword2
      let params = { params: { tolevel: 5, pagestart: this.pagestart2, limit: this.limit } }
      if (typeof keyword !== 'undefined' && keyword && this.$util.trim(keyword) !== '') {
      ...
      this.$http.get(`${ENV.BokaApi}/api/retailer/customerList`, params).then(res => {
        let data = res.data
        this.$vux.loading.hide()
        this.tabcount2 = data.count
        let retdata = data.data ? data.data : data
        for (var i = 0; i < retdata.length; i++) {
          retdata[i].checked = false
          retdata[i].dateline_str = new Time(retdata[i].dateline * 1000).dateFormat('yyyy-MM-dd')
    ...
    getData3 () {
      this.$vux.loading.show()
      let keyword = this.searchword3
      let params = { params: { tolevel: 100, pagestart: this.pagestart3, limit: this.limit } }
      if (typeof keyword !== 'undefined' && keyword && this.$util.trim(keyword) !== '') {
        this.searchresult3 = true
        params.params.keyword = keyword
      } else {
      ...
      switch (this.selectedIndex) {
        case 0:
          if (this.tabdata1.length < this.limit) {
            ...
            this.distabdata1 = false
            this.tabdata1 = []
            if (!this.orderbyParams || !this.orderbyParams.orderby) {
...
.sale-customer-list-page .qbtnInfo{
...
.sale-customer-list-page .condition{
  position: relative;
  height: 40px;
  line-height: 40px;
  border-bottom: 1px solid #eeeeee;
}
.sale-customer-list-page .h35{
  height: 35px;
  line-height: 35px;
}
