  ...
  <div class="containerarea s-havebottom font14 rnews">
        ...
        <swiper-item v-for="(item, index) in tabtxts" :key="index">
          <template v-if="(index == 0)">
            <div class="scroll_list pl10 pr10">
              <Listplate1 v-for="(item,index1) in tabdata1" :key="index1">
          ...
          <template v-if="(index == 1)">
            <div class="scroll_list pl10 pr10">
              <Listplate1 v-for="(item,index2) in tabdata2" :key='index2'>
                <img slot="pic" :src="item.photo" style="width:40px;height:40px;" />
    ...
    <div v-transfer-dom>
      <popup class="menuwrap" v-model="showpopup1" @on-hide="popupevent('hide')" @on-show="popupevent('show')">
        <div class="popup0">
          <div class="list">
            <template v-for="(row,index) in controldata1">
              <div class="item flex_center" @click="clickpopup1(row.key,clickdata1)" :key="index">
    ...
    <div v-transfer-dom>
      <popup class="menuwrap" v-model="showpopup2" @on-hide="popupevent('hide')" @on-show="popupevent('show')">
        <div class="popup0">
          <div class="list">
            <template v-for="(row,index) in controldata2">
              <div class="item flex_center" @click="clickpopup2(row.key,clickdata2)" :key="index">
                <div :class="`clamp1 ${row.key}`">{{ row.title }}</div>
              </div>
            </template>
